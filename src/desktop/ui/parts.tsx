...
export function ExternalLink({
  ...
  return (
    <a
      ...
      className={
        className
          ? [className, 'hover:underline focus:outline-none'].filter(Boolean).join(' ')
          : 'text-primary hover:text-accent hover:underline focus:outline-none'
      }
      href={href}
      aria-label={label}
    >
...
export function Divider({ className }: { className?: string }) {
  return (
    <div
      className={['w-full border-b border-gray-300 dark:border-gray-600', className]
        .filter(Boolean)
        .join(' ')}
      role="separator"
    />
...
const SOCIAL_ICONS: Record<SocialPlatform, { viewBox: string; path: string }> = {
  twitter: {
    viewBox: '0 0 16 16',
...
export function SocialMediaIcons() {
  return (
    <ul className="flex flex-wrap items-center gap-x-2 gap-y-2 fill-current text-secondary">
      {SOCIAL_LINKS.map((link) => {
        const icon = SOCIAL_ICONS[link.platform];
        return (
          <li key={link.platform}>
            <a
              className="inline-block p-2 hover:text-accent"
              href={link.url}
              target="_blank"
              rel="noopener noreferrer"
            >
