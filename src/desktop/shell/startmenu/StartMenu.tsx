...
export default function StartMenu({
  ...
  return (
    <motion.div
      ref={menuRef}
      className={`fixed z-[110] flex w-[min(16.5rem,calc(100vw-1rem))] flex-col ${DESKTOP_OVERLAY_SURFACE} ${DESKTOP_OVERLAY_SHADOW} max-sm:max-h-[min(24rem,calc(100dvh-4rem))] max-sm:w-[min(18rem,calc(100vw-0.75rem))]`}
      role="menu"
    ...
    >
      <header className="flex items-center gap-[0.625rem] border-b border-[rgb(113_113_122/0.3)] bg-[linear-gradient(180deg,rgb(113_113_122/0.16)_0%,rgb(113_113_122/0.06)_100%)] px-3 py-[0.625rem] dark:bg-[linear-gradient(180deg,rgb(161_161_170/0.12)_0%,rgb(24_24_27/0.4)_100%)]">
        <span
          className="flex h-8 w-8 shrink-0 items-center justify-center border border-[color:var(--color-hairline)] bg-[var(--color-control-fill-strong)] text-[1.125rem] leading-none"
          aria-hidden="true"
        >
          🧐
        </span>
        <div className="flex min-w-0 flex-col gap-[0.125rem]">
          ...
          <span className="overflow-hidden text-[0.5625rem] text-ellipsis whitespace-nowrap text-muted">
            guest@alfon.so · devfolio v1.0
          </span>
        </div>
      </header>
      ...
      >
        <button
          type="button"
          role="menuitem"
          className="block w-full cursor-pointer border border-[color:var(--color-hairline)] bg-[var(--color-control-fill-strong)] px-1 py-[0.35rem] font-[inherit] text-[0.5625rem] leading-[1.2] text-secondary hover:border-[color:var(--color-highlight-border)] hover:bg-[var(--color-highlight-bg)] hover:text-primary hover:outline-none focus-visible:border-[color:var(--color-highlight-border)] focus-visible:bg-[var(--color-highlight-bg)] focus-visible:text-primary focus-visible:outline-none"
