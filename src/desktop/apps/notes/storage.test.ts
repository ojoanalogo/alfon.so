...
describe('notes/storage', () => {
  ...
  describe('save/load round-trip', () => {
    ...
    it('round-trips an empty array', () => {
      ...
      expect(loadNotes()).toEqual([]);
      expect(localStorage.getItem(storageKey('notes'))).toBe('[]');
    });
    ...
    it('writes the value under the expected storage key', () => {
      ...
      saveNotes(notes);
      expect(JSON.parse(localStorage.getItem(storageKey('notes')) as string)).toEqual(notes);
    });
  });

  describe('malformed / invalid stored data resilience', () => {
    it('returns [] when stored JSON is malformed', () => {
      localStorage.setItem(storageKey('notes'), '{not valid json');
      expect(loadNotes()).toEqual([]);
    });

    it('returns [] when stored JSON is not an array (object)', () => {
      localStorage.setItem(storageKey('notes'), JSON.stringify({ id: 'a' }));
      expect(loadNotes()).toEqual([]);
    });

    it('returns [] when stored JSON is a primitive', () => {
      localStorage.setItem(storageKey('notes'), JSON.stringify('just a string'));
      expect(loadNotes()).toEqual([]);
    });
    ...
    it('filters out entries that are not valid notes', () => {
      ...
      localStorage.setItem(storageKey('notes'), JSON.stringify(mixed));
      const loaded = loadNotes();
    ...
    it('drops a note missing a required string field', () => {
      ...
      localStorage.setItem(storageKey('notes'), JSON.stringify(arr));
      expect(loadNotes()).toEqual([]);
    });

    it('keeps notes with empty-string fields (still valid strings)', () => {
      const note = makeNote({ id: '', title: '', content: '', updatedAt: '' });
      localStorage.setItem(storageKey('notes'), JSON.stringify([note]));
